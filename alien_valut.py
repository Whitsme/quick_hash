...
parser = OTXv2.argparse.ArgumentParser(description='OTX CLI Example')
...
if args['ip']:
    alerts = OTXv2.get_malicious.ip(otx, args['ip'])
    if len(alerts) > 0:
...
if args['host']:
    alerts = OTXv2.get_malicious.hostname(otx, args['host'])
    if len(alerts) > 0:
        print('Identified as potentially malicious')
        print(str(alerts))
    else:
...
if args['url']:
    alerts = OTXv2.get_malicious.url(otx, args['url'])
    if len(alerts) > 0:
        print('Identified as potentially malicious')
        print(str(alerts))
    else:
...
if args['hash']:
    alerts =  OTXv2.get_malicious.file(otx, args['hash'])
    if len(alerts) > 0:
        print('Identified as potentially malicious')
        print(str(alerts))
    else:
...
if args['file']:
    hash = OTXv2.hashlib.md5(open(args['file'], 'rb').read()).hexdigest()
    alerts =  OTXv2.get_malicious.file(otx, hash)
    if len(alerts) > 0:
        print('Identified as potentially malicious')
        print(str(alerts))
    else:
